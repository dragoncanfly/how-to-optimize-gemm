                ...
                double* b, int ldb, double* c, int ldc)
{
    ...
    for (j = 0; j < n; j+= 4) //Loop over the columns of C, unrolled by 4
    {
        ...
        for (i = 0; i < m; i+=4) //Loop over the rows of C
        {
            ...
            AddDot4x4(k, &packedA[i*k], lda, &packedB[j*k], 4, &C(i, j), ldc);
        }
    }
}
...
void AddDot4x4(int k, double* a, int lda, double* b, int ldb, double* c, int ldc)
{
    ...
    *a_0p_pntr, *a_1p_pntr, *a_2p_pntr, *a_3p_pntr;
    ...
    for (p = 0; p < k; p++)
    {
        ...
        a_0p_vreg.v = _mm_loaddup_pd((double*)a);
        a_1p_vreg.v = _mm_loaddup_pd((double*)a+1);
        a_2p_vreg.v = _mm_loaddup_pd((double*)a+2);
        a_3p_vreg.v = _mm_loaddup_pd((double*)a+3);
        a += 4;
